...
function runStage1()
 ...
 while true do
  if isStage1DoneByOpenColor() then
   phase("Stage 1 xong - màu Open")
   return true
  end
  ...
  if isTikTokLiteInstalled() then
   phase("Stage 1 xong - TTL đã cài")
   return true
  end
  if handleError1UntilClear() then phase("Xử lý lỗi 1 xong") else
   if findImage(CLOUD_IMG, 82, 0, 0, 750, 1334) then
    ...
    -- Đã bấm tải mà cloud biến mất nhiều vòng => AppStore đang ở trạng thái tải/xong tải; tránh kẹt vô hạn.
    if noCloudCount >= 20 then
     phase("Stage 1 xong - cloud biến mất, chuyển LoginGG")
     return true
    end
    waitPhase(1000)
   end
...
-- ===== AUTO CONTINUE: LOGIN GG =====
pcall(function() local ok,wv=pcall(require,'webview'); if ok and wv and wv.remove then wv.remove(88) end end)\nlocal f=io.open('/var/mobile/Media/1ferver/log/stage1_to_login_marker.txt','w'); if f then f:write(os.date('%H:%M:%S')..' DOFILE_LOGIN_NO_DELAY\n'); f:close() end\ndofile('/var/mobile/Media/1ferver/lua/scripts/LoginGGSafari_Stage6_runtime.lua')
