...
class XXTouchOpenAPIClient:
    ...
    def command_spawn(self, command: str):
        # XXTouch /command_spawn expects raw shell text. Sending JSON makes dash try to
        # execute a literal string like {"command":"..."}, returning status 127.
        return self._post_raw('/command_spawn', str(command or ''), content_type='text/plain; charset=utf-8')


    def select_script_file(self, script_path: str):
        return self._post_json('/select_script_file', {'path': script_path, 'filename': script_path})

    def launch_script_file(self):
        return self._post_json('/launch_script_file', {})
