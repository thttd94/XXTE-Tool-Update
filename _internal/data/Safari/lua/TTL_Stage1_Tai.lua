...
function openTikTokLiteStore()
 ...
 -- inline combo: skip waitPostOpenColor5s() to avoid AppStore screen API interrupt
 waitPhase(3000)
 app.open_url(TIKTOK_LITE_STORE_URL)
 -- inline combo: skip waitPostOpenColor5s() to avoid AppStore screen API interrupt
end
...
function isStage1DoneByOpenColor()
...
function isOpenButtonVisible()
 -- Chuẩn Stage1 DONE: AppStore hiện nút Open. Match nhiều ngưỡng/vùng vì ảnh openTTL có thể lệch theme/ngôn ngữ.
 local tries = {
  {82,0,0,750,1334},{78,0,0,750,1334},{72,0,0,750,1334},{66,0,0,750,1334},
  {72,430,160,740,520},{66,430,160,740,520},{62,400,120,750,620}
 }
 for _,t in ipairs(tries) do
  local ok = findImage(OPEN_IMG, t[1], t[2], t[3], t[4], t[5])
...
function isTikTokLiteInstalled()
 local ok,path = pcall(function() return app.bundle_path(TIKTOK_LITE_BUNDLE) end)
 if ok and path and tostring(path) ~= "" and tostring(path) ~= "nil" then
  return true
 end
 return false
end
...
function runStage1()
 ...
 local noCloudCount = 0
 while true do
  -- Không dùng màu xanh/bundle_path để kết thúc: cloud/download cũng có màu xanh và bundle_path có thể true khi app còn placeholder.
  -- Chỉ chuyển LoginGG khi App Store hiện đúng nút Open (openTTL.png).
  if isOpenButtonVisible() then
   phase("Stage 1 xong - thấy Open")
   return true
  end
  if handleError1UntilClear() then phase("Xử lý lỗi 1 xong") else
   if screen.is_colors({{337,319,0x007aff},{336,313,0x007aff}}, 90) then
    ...
    touch.tap(337,319)
    waitPhase(2000)
   elseif findImage(CLOUD_IMG, 82, 0, 0, 750, 1334) then
    noCloudCount = 0
    tapByImageCenter(CLOUD_IMG, 82, 0, 0, 750, 1334)
    waitPhase(2000)
   else
    noCloudCount = noCloudCount + 1
    phase("Đang tải / chờ Open " .. tostring(noCloudCount))
    -- Cloud biến mất CHƯA có nghĩa là tải xong; có thể App Store mới bắt đầu download.
    -- Chỉ chuyển LoginGG khi thấy nút Open hoặc bundle TikTok Lite đã cài thật.
    if noCloudCount % 30 == 0 then
     phase("Vẫn đang chờ TTL tải/cài xong " .. tostring(noCloudCount) .. "s")
    end
    if noCloudCount >= 600 then
     phase("Chờ tải quá 10p - mở lại App Store kiểm tra")
     openTikTokLiteStore()
     noCloudCount = 0
    end
    waitPhase(1000)
