screen.init(0)
local __WEBVIEW_STATUS_TEXT = "Nháº­p TK Lite Äang cháº¡y ..."
...
</style></head><body><div id="bar">Nháº­p TK Lite Äang cháº¡y ...</div></body></html>
]]
...
local IN_SANDBOX_TGZ = ARCHIVE_DIR .. "/lite_sandbox.tgz"
...
os.execute("killall -9 TikTokLite TikTok Lite cfprefsd 2>/dev/null || true")
sys.msleep(1500)
...
local function importFile(label, src, dst)
 local d=readAll(src); if not d then status(label.." nguá»“n miss",1200); return false end
 if not writeAll(dst,d) then status(label.." write lá»—i",1200); return false end
 status(label.." OK",700); return true
end
local function q(s) return "'" .. tostring(s):gsub("'", "'\''") .. "'" end
local tgz=io.open(IN_SANDBOX_TGZ,"rb")
if tgz then
 tgz:close()
 os.execute("mkdir -p "..q(dataPath).." 2>/dev/null")
 local rc=os.execute("tar -xzf "..q(IN_SANDBOX_TGZ).." -C "..q(dataPath).." 2>/dev/null")
 local f=io.open(dataPath.."/Documents/ttaccountSDKUserInfo.archiver","rb")
 if f then f:close(); ok=ok+1; table.insert(lines,"sandbox_restore=ok") else table.insert(lines,"sandbox_restore_rc="..tostring(rc)) end
end
local files = {
 {"cookies", OUT_COOKIES, dataPath.."/Library/Cookies/Cookies.binarycookies"},
 {"archiver", OUT_ARCHIVER, dataPath.."/Documents/ttaccountSDKUserInfo.archiver"},
 {"plist", OUT_PLIST, dataPath.."/Library/Preferences/com.ss.iphone.ugc.tiktok.lite.plist"},
}
for _,it in ipairs(files) do if importFile(it[1], it[2], it[3]) then ok=ok+1; table.insert(lines,it[1].."="..it[3]) end end
-- Đảm bảo TikTok Lite đọc được file sau restore; tránh trạng thái phải nạp nhiều lần.
os.execute("chown -R mobile:mobile "..q(dataPath).."/Documents "..q(dataPath).."/Library/Cookies "..q(dataPath).."/Library/Preferences 2>/dev/null || true")
os.execute("chmod 644 "..q(dataPath).."/Library/Cookies/Cookies.binarycookies "..q(dataPath).."/Documents/ttaccountSDKUserInfo.archiver "..q(dataPath).."/Library/Preferences/com.ss.iphone.ugc.tiktok.lite.plist 2>/dev/null || true")
os.execute("sync")
os.execute("killall -9 TikTokLite cfprefsd 2>/dev/null || true")
sys.msleep(2500)
writeAll(OUT_STATUS, "ok="..tostring(ok).."/3\ndataPath="..tostring(dataPath).."\n"..table.concat(lines,"\n").."\n")
status("Nháº­p Lite xong "..tostring(ok).."/3", 2500)
